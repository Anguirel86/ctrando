...
# TODO: Weapon graphics data @ $CD4024.  15 byte records, 5 bytes for
#       melee/range/crit.
...
import functools
import typing
from typing import ByteString, Optional, Self

from ctrando.common import byteops, ctenums, ctrom
from ctrando.common.ctenums import WeaponEffects, ArmorEffects
...
_weapon_effect_abbrev_dict: dict[WeaponEffects, str] = {
    ...
    WeaponEffects.MP_CRIT4X: "20MP Crit4x",
    WeaponEffects.HP_LEECH_5: "+5% HP Hit",
    WeaponEffects.HP_LEECH_10: "+10% HP Hit",
    WeaponEffects.MP_LEECH_2: "+2 MP Hit",
    WeaponEffects.MP_LEECH_5: "+5 MP Hit",
}
...
class WeaponStats(ItemData):
    ...
    def has_effect(self, val: bool):
        if not val:
            self.effect_id = WeaponEffects.NONE
...
Buff = Type_05_Buffs | Type_06_Buffs | Type_08_Buffs | Type_09_Buffs
BuffIterable = (
        Iterable[Type_05_Buffs] | Iterable[Type_06_Buffs] |
        Iterable[Type_09_Buffs] | Iterable[Type_08_Buffs]
)


def get_buff_string(buffs: typing.Union[Buff, BuffIterable]) -> str:
    """
...
class AccessoryStats(ItemData):
    ...
    def _get_buff_type_index(self, buff: Buff) -> int:
    ...
    @property
    def has_hp_mod(self) -> bool:
        return bool(self._data[1] & 0x10)

    @has_hp_mod.setter
    def has_hp_mod(self, val: bool):
        if val:
            self._data  [1] |= 0x10
        else:
            self._data[1] &= (0xFF - 0x10)
    ...
    @property
    def has_mp_mod(self) -> bool:
        return bool(self._data[0] & 0x20)

    @has_mp_mod.setter
    def has_mp_mod(self, val: bool):
        if val:
            self._data[0] |= 0x20
        else:
            self._data[0] &= (0xFF-0x20)
...
def get_accessory_desc_str(
    ...
    if stats.has_stat_boost:
        boost_ind = stats.stat_boost_index
        boost = stat_boosts[boost_ind]
        boost_str = boost.stat_string()

    if stats.has_hp_mod:
        hp_str = f"+{stats.hp_mod}% HP"

    if stats.has_mp_mod:
        mp_str = f"-{stats.mp_mod}% MP Cost"

    if stats.has_counter_effect:
        type_str = "basic atk" if stats.has_normal_counter_mode else "ATB fill"
...
class Item:
    ...
    @classmethod
    def from_rom(cls, rom: ByteString, item_id: ctenums.ItemID):
...
class ItemDB:
    def __init__(self,
        ...
        self.item_dict: dict[ctenums.ItemID, Item] = dict(item_dict)
    ...
    def base_mp_healing(self, val: int):
        ...
        healing_item_ids = (x for x in list(ctenums.ItemID)
                            if ctenums.ItemID.TONIC <= x <= ctenums.ItemID.LAPIS)
    ...
    @classmethod
    def from_rom(cls, rom: typing.ByteString) -> Self:
        item_dict = {
    ...
    def update_description(self, item_id):
        ...
        if isinstance(item.stats, AccessoryStats):
            if item_id == IID.WALLET:
                pass
            else:
                main_desc = get_accessory_desc_str(item.stats, self.stat_boosts)
    ...
    def write_to_ctrom(self, ct_rom: ctrom.CTRom):
        ...
        ct_rom.seek(0x02F318)
        ct_rom.write(int.to_bytes(write_pos & 0x00FFFF, 2, 'little'))
        ct_rom.seek(0x02F31E)
        ct_rom.write(bytes([bank_byte]))
        ...
        for i in range(0xF2):
            ...
            ct_rom.seek(ptr_pos)
            ct_rom.write(local_ptr.to_bytes(2, 'little'), FSW.MARK_USED)
            ptr_pos += 2

            if i in self.item_dict:
                item_id = ctenums.ItemID(i)
                ct_rom.seek(desc_pos)
                desc = self.item_dict[item_id].desc
                ct_rom.write(desc, FSW.MARK_USED)
                desc_pos += len(desc)

        ct_rom.seek(0x01DA6B)
        ct_rom.write(int.to_bytes(self.base_hp_healing, 2, 'little'))

        ct_rom.seek(0x02B180)
        ct_rom.write(int.to_bytes(0x80 | self.base_hp_healing, 1, 'little'))

        ct_rom.seek(0x01DAE4)
        ct_rom.write(int.to_bytes(self.base_mp_healing, 2, 'little'))

        ct_rom.seek(0x01DB09)
        ct_rom.write(int.to_bytes(self.base_mp_healing, 2, 'little'))

        ct_rom.seek(0x02B187)
        ct_rom.write(int.to_bytes(0x80 | self.base_mp_healing, 1, 'little'))
